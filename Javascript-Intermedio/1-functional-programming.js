...
// -----> EXERCISE 2: Understand Functional Programming Terminolog <-----
// Function that returns a string representing a cup of green tea
...
// The global variable
let fixedValue2= 4; // En la resolución original la variable es "fixedValue". Lo cambio acá para evitar el error que indica el editor de código al haber duplicados
...
// The global variable (the same as exercise 7)
...
// The global variable (the same as exercise 7)
  ...
  function getRating(watchList) {
    ...
    const byChristopherNolan = watchList.filter( movie => movie.Director == 'Christopher Nolan');
    
    let averageRating = 0;
  
    let sumRating = byChristopherNolan.reduce((acumRating, actRating) =>{
      return acumRating + parseFloat(actRating.imdbRating); 
    }, 0);
  
    averageRating = sumRating / byChristopherNolan.length;
    // Only change code above this line
...
const squareList = arr => {
    // Only change code below this line
    const square = arr.filter( element => element > 0 && Number.isInteger(element))
                            .map( element =>Math.pow(element, 2)); // Otras formas: (element * element) o (element ** 2) 
...
function urlSlug(title) {
    const noSpace = title.trim(); // Elimina los espacios en los extremos
    const arr = noSpace.split(/\s* /);
    const str = arr.join('-');
...
function checkPositive(arr) {
    // Only change code below this line
    const bool = arr.every(elemen => elemen > 0);
    return bool;
    // Only change code above this line
  }
...
function checkPositive(arr) {
    // Only change code below this line
    const somePositive = arr.some(value => value > 0);
    return somePositive;
    // Only change code above this line
  }
